							...
							if (lock3)
							{
								BX.bind(lock3, "click", function(){
									B24.licenseInfoPopup.show('network-available', '<?=GetMessageJS("CONFIG_NETWORK_AVAILABLE_TITLE")?>',
										'<?=GetMessageJS("CONFIG_NETWORK_AVAILABLE_TEXT_NEW", array("#PRICE#" => $arResult["PROJECT_PRICE"]))?>');
								});
							}
						});
					</script>
	...
	<!-- features -->
		...
		<tr>
			...
			<td class="content-edit-form-field-input" colspan="2">
				...
				<!--<input type="checkbox" name="feature_processes" id="feature_processes" <?if (COption::GetOptionString("lists", "turnProcessesOn") == "Y") echo "checked"?>/>
				<label for="feature_processes"><?=GetMessage("CONFIG_FEATURES_PROCESSES")?></label><br/>-->
			</td>
		</tr>
	<!--ip -->
		<?
...
<script>
	...
	<?if ($arResult["LICENSE_TYPE"] == "team" || $arResult["LICENSE_TYPE"] == "company" || $arResult["LICENSE_TYPE"] == "nfr"):?>
