...
if ($arParams['IS_AJAX'] == 'Y')
{
	...
	echo '<script type="text/javascript" src="', $this->getFolder(), '/script.js?v11"></script>';
}
?>
...
<div class="crm-activity-todo-item<?= $item['COMPLETED']=='Y' ? ' crm-activity-todo-item-completed' : ''?>"<?
	...
	?> data-icon="<?= $item['ICON']?>">
		...
		<label class="crm-activity-todo-item-middle" for="check<?= $item['ID']?>">
			...
			<?endif;?>
			<a href="<?= $uriView->getUri();?>" data-id="<?= $item['ID']?>" class="crm-activity-todo-link"><?= $item['SUBJECT']?></a>
