...
/*
if ($arResult['DISK_SPACE'] > 0)
{
	//$trashcanWillAvailableSizeFormat = \CFile::formatSize($arResult['DISK_SPACE'] - $arResult['DROP_TRASHCAN']);
	//$unnecessaryVersionWillAvailableSizeFormat = \CFile::formatSize($arResult['DISK_SPACE'] - $arResult['DROP_UNNECESSARY_VERSION']);
	$totalWillAvailableSizeFormat = \CFile::formatSize($arResult['DISK_SPACE'] - $arResult['DROP_TOTAL_SIZE']);
}
else
{
	//$trashcanWillAvailableSizeFormat = $arResult['DROP_TRASHCAN_FORMAT'];
	//$unnecessaryVersionWillAvailableSizeFormat = $arResult['DROP_UNNECESSARY_VERSION_FORMAT'];
	$totalWillAvailableSizeFormat = $arResult['DROP_TOTAL_SIZE_FORMAT'];
}
*/
?>
...
<?
	...
	else
	{
		if ($arResult["ADMIN_MODE"])
		{
			...
			<div class="disk-volume-header-container">
				...
				<div class="disk-volume-header-block">
					...
					<?/*<div id="bx-disk-volume-total-disk-size" class="disk-volume-header-amount-info disk-volume-hint" off-data-hint="total_usage">*/?>
					...
					<? /* if ($arResult['DISK_SPACE'] > 0): ?>
						<a href="#" class="disk-volume-header-link"><?= Loc::getMessage('DISK_VOLUME_EXPAND_VOLUME'); ?></a>
					<? endif; */ ?>
				</div>
			</div>
			<?
		...
		if (count($arResult[$plotIndicatorId]['LIST']) > 0)
		{
			...
			$tt = 0;
			$legend = array();
			foreach ($arResult[$plotIndicatorId]['LIST'] as $row)
			{
		...
		{
			?>
			<?/*<h1 class="disk-volume-title disk-volume-hint" off-data-hint="safe_clear"><?= Loc::getMessage('DISK_VOLUME_MAY_DROP_TITLE') ?></h1>*/?>
			...
			<div class="webform-buttons pinable-block ">
				<div id="disc-volume-space-selector" class="disc-volume-space-entity-container">
					<? if ($arResult['DROP_UNNECESSARY_VERSION'] > 0): ?>
						<div id="disc-volume-space-unnecessaryVersion" data-checked="N" data-type="unnecessaryVersion" class="disc-volume-space-entity-block <?
							...
							<div class="disc-volume-space-entity-inner">
								<!--<div class="task-edit-add-template-checkbox" id="bx-disk-drop-free-version"></div>-->
					...
					<? if ($arResult['DROP_TRASHCAN'] > 0): ?>
						<div id="disc-volume-space-trashCan" data-checked="N" data-type="trashcan" class="disc-volume-space-entity-block <?
							...
							<div class="disc-volume-space-entity-inner">
								<!--<div class="task-edit-add-template-checkbox" id="bx-disk-drop-trashcan"></div>-->
				...
				<div class="disk-volume-button-container">
					...
					<?/*
					<a href="<?= $component->getActionUrl(array('expert' => 'off', 'reload' => 'Y', 'action' => $component::ACTION_DEFAULT)); ?>" class="webform-small-button disk-volume-reload-link">
						<?= Loc::getMessage("DISK_VOLUME_MEASURE_DATA_REPEAT");?>
					</a>

					<a href="<?= $component->getActionUrl(array('expert' => 'on', 'action' => ($arResult["ADMIN_MODE"] ? $component::ACTION_DISKS : $component::ACTION_STORAGE), 'storageId' => '')); ?>" class="disk-volume-link-grey"><?= Loc::getMessage("DISK_VOLUME_EXPERT_MODE"); ?></a>
					*/?>
				</div>

				<div class="disc-volume-notice-small">
					<span id="bx-disk-volume-space" style="height:0;display:none">
						<span id="bx-disk-volume-space-amount" class="disk-volume-space-amount-small"><?//= $totalWillAvailableSizeFormat ?></span>
...
<script type="text/javascript">
	BX(function () {
		...
		/*BX.bind(startMeasureButton, 'click', function(e)
		{
