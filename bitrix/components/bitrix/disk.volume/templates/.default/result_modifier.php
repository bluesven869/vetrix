...
if (!isset($fileTitleFormat) || !is_callable($fileTitleFormat))
{
	...
	$fileTitleFormat = function (&$row, &$storage) use ($arParams, $arResult, $component)
	{
		...
		)
		{
			...
			if ($entityType == \Bitrix\Im\Disk\ProxyType\Im::className())
			{
				$titleFormat =
					'<span class="bx-file-icon-container-small bx-file-icon-tiny '.$iconClass.'"></span>'.
					//'<span class="bx-file-icon-container-small '.$iconClass.'"></span>'.
					'<span class="bx-disk-folder-title">'.$row['TITLE'].'</span>'.
			...
			else
			{
				...
				$titleFormat =
					'<span class="bx-file-icon-container-small bx-file-icon-tiny '.$iconClass.'"></span>'.
					//'<span class="bx-file-icon-container-small '.$iconClass.'"></span>'.
					'<span class="bx-disk-folder-title" '. $parentsHint. '>'.$row['TITLE'].'</span>';
		...
		)
		{
			...
			if ($row['USING_COUNT'] > 0)
			{
				...
				if ($file instanceof \Bitrix\Disk\File)
				{
					...
					/*
					if ($fragment->getSharingCount() > 0)
					{
						//$fileSharing = $file->getSharingsAsLink();
						$usingCount += $fragment->getSharingCount();
						if ($usingCountMenuLinks != '')
							$usingCountMenuLinks .= ', ';
						if ($fragment->getSharingCount() > 1)
						{
							$usingCountMenuLinks .=
								'{text: \''.Loc::getMessage('DISK_VOLUME_SHARING_COUNT').': '.$fragment->getSharingCount().'\'}';
						}
						else
						{
							$usingCountMenuLinks .=
								'{text: \''.Loc::getMessage('DISK_VOLUME_SHARING_COUNT').'\'}';
						}
					}
					*/
				}
...
switch ($arResult['ACTION'])
{
	...
	case $component::ACTION_FILES:
	{
		...
		foreach ($arResult[$resId]['LIST'] as &$row)
		{
			...
			if ($isTrashcan)
			{
				$row['ACTIONS'] = $component->getDeletedFileActionMenu($row, $currentStorage);
			}
			else
			{
...
if (!empty($arResult['MODULES']['LIST']))
{
	foreach ($arResult['MODULES']['LIST'] as &$row)
	{
		try
		{
			$component->decorateResult($row);
