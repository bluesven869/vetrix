...
if ($isBitrix24Template)
{
	...
	<div class="pagetitle-container pagetitle-flexible-space" style="overflow: hidden;">
		...
		)
		{
			$APPLICATION->IncludeComponent(
				...
				'',
				array(
					//"RENDER_FILTER_INTO_VIEW" => 'inside_pagetitle',
					//"RENDER_FILTER_INTO_VIEW_SORT" => 100,
	...
	if ($arResult["ADMIN_MODE_ALLOW"] || $arResult["WORKER_COUNT"] > 0)
	{
		...
		<div id="bx-disk-volume-menu" class="pagetitle-container pagetitle-align-right-container">
			<?
			/*
			if (!$arResult["EXPERT_MODE"])
			{
				$expertTitle = Loc::getMessage('DISK_VOLUME_EXPERT_MODE_ON');
				if ($arResult["ADMIN_MODE"])
				{
					$expertLink = $component->getActionUrl(array('expert' => 'on', 'action' => $component::ACTION_DISKS, 'storageId' => ''));
				}
				else
				{
					$expertLink = $component->getActionUrl(array('expert' => 'on', 'action' => $component::ACTION_STORAGE, 'storageId' => ''));
				}
			}
			else
			{
				$expertTitle = Loc::getMessage('DISK_VOLUME_EXPERT_MODE_EXIT');
				$expertLink = $component->getActionUrl(array('expert' => 'off', 'action' => $component::ACTION_DEFAULT, 'storageId' => ''));
			}

			?>
			<span class="webform-small-button webform-small-button-transparent webform-small-button-check-round" title="<?= $expertTitle ?>">
				<a class="webform-small-button-text" href="<?= $expertLink ?>">
					<? if ($arResult["EXPERT_MODE"]): ?>
						<span class="webform-small-button-icon"></span>
					<? endif; ?>
					<?= GetMessageJS('DISK_VOLUME_EXPERT_MODE'); ?>
				</a>
			</span>
			<?
			*/
			/*
			<div class="webform-small-button webform-small-button-transparent disk-volume-menu-reload-button" title="">
				<a id="bx-disk-volume-link-reload" class="webform-small-button-text" href="">
					<span class="webform-small-button-icon"></span>
				</a>
			</div>
			*/
			...
			BX.ready(function()
			{
				...
				<?/* if (!$arResult["EXPERT_MODE"]): ?>
					<? if ($arResult["ADMIN_MODE"]): ?>
						menuItemsOptions.push({
							text: '<?=GetMessageJS('DISK_VOLUME_EXPERT_MODE')?>',
							href: '<?= $component->getActionUrl(array('expert' => 'on', 'action' => $component::ACTION_DISKS, 'storageId' => '')); ?>'
						});
					<? else: ?>
						menuItemsOptions.push({
							text: '<?=GetMessageJS('DISK_VOLUME_EXPERT_MODE')?>',
							href: '<?= $component->getActionUrl(array('expert' => 'on', 'action' => $component::ACTION_STORAGE, 'storageId' => '')); ?>'
						});
					<? endif; ?>
				<? else: ?>
					menuItemsOptions.push({
						text: '<?=GetMessageJS('DISK_VOLUME_EXPERT_MODE_EXIT')?>',
						href: '<?= $component->getActionUrl(array('expert' => 'off', 'action' => $component::ACTION_DEFAULT)); ?>'
					});
				<? endif; */?>
				...
				//var buttonRect = BX("bx-disk-volume-popupMenuOptions").getBoundingClientRect();
				var menu = BX.PopupMenu.create(
					...
					{
						closeByEsc: true,
						offsetLeft: 20,//(buttonRect.width / 2),
						angle: true
					}
				);
...
if ($isBitrix24Template && !$arResult['DISK_EMPTY'] && !$isQueueRunning && $arResult['DATA_COLLECTED'])// && !empty($arResult['MENU_ITEMS']))
{
	...
	<div class="tasks-view-switcher pagetitle-align-right-container">
		<div id="disk-volume-filter-preset-buttons" class="disk-volume-menu-right">
			<?
			$APPLICATION->includeComponent(
				"bitrix:main.interface.buttons",
				"",
				array(
					"ID" => "disk-volume-filter-preset-buttons",
...
{
	?>
	<div class="disk-volume-info-control-panel">
		<div
			class="disk-volume-info-control-panel-warning"><?= Loc::getMessage('DISK_VOLUME_ADMIN_MODE_COMMENT'); ?></div>
		<a href="<?= $component->getActionUrl(array('action' => $component::ACTION_DEFAULT, 'admin' => 'off')); ?>" class="disk-volume-info-control-panel-link"><?= Loc::getMessage('DISK_VOLUME_ADMIN_MODE_EXIT'); ?></a>
	</div>
	<?
}
*/
		...
		if ($component::ACTION_DISKS === $arResult['ACTION'] && $arResult["ADMIN_MODE"])
		{
			...
			<div class="disk-volume-header-container">
				...
				<div class="disk-volume-header-block">
					...
					<?/* <div id="bx-disk-volume-total-disk-size" class="disk-volume-header-amount-info disk-volume-hint" data-hint="total_usage">*/?>
					...
					<?/*<div id="bx-disk-volume-total-disk-count" class="disk-volume-header-amount-info disk-volume-hint" off-data-hint="total_count">*/?>
			...
			<div class="disk-volume-header-container <?=($arResult['Storage']['IS_EXTRANET'] ? 'disk-volume-header-container-extranet' : '')?>">
				...
				<div class="disk-volume-header-block">
					...
					<? else: ?>
						...
						<?/*<div id="bx-disk-volume-total-disk-size" class="disk-volume-header-amount-info disk-volume-hint" off-data-hint="disk_total_usage">*/?>
						<div id="bx-disk-volume-total-disk-size" class="disk-volume-header-amount-info">
							<? if ($arResult['TOTAL_FILE_SIZE'] > 0): ?>
								<?= Loc::getMessage("DISK_VOLUME_DISK_TOTAL_USEAGE", array('#FILE_SIZE#' => $arResult['TOTAL_FILE_SIZE_FORMAT'])) ?>
							<? endif ?>
						...
						<?/*<div id="bx-disk-volume-total-disk-count" class="disk-volume-header-amount-info disk-volume-hint" off-data-hint="disk_total_count">*/?>
						<div id="bx-disk-volume-total-disk-count" class="disk-volume-header-amount-info">
							<? if ($arResult['TOTAL_FILE_COUNT'] > 0): ?>
								<?= Loc::getMessage("DISK_VOLUME_DISK_TOTAL_COUNT", array('#FILE_COUNT#' => $arResult['TOTAL_FILE_COUNT'])) ?>
							<? endif ?>
...
if (!empty($arResult['MENU_ITEMS']))
{
	...
	<div class="tasks-view-switcher pagetitle-align-right-container">
		<div id="disk-volume-filter-preset-buttons" class="disk-volume-menu-right">
			<?
			// remove css class 'top-menu-mode'
