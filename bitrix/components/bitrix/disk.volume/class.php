...
class CDiskVolumeComponent extends BaseComponent
{
	...
	protected function processActionDefault()
	{
		try
		{
			...
			if ($this->isOnlySpecificStorage() || !$this->isAdminMode())
			{
				...
				$this->arResult['DISK_SPACE'] = Volume\Storage\Storage::getAvailableSpace($this->storage);
			}
			elseif ($this->isAdminMode())
			{
				...
				// full disk limit
				$this->arResult['DISK_SPACE'] = Volume\Storage\Storage::getAvailableSpace();
			}

			if ($this->arResult['DISK_SPACE'] > 0)
			{
				$this->arResult['DISK_SPACE_FORMAT'] = \CFile::formatSize($this->arResult['DISK_SPACE']);
			}


			$this->arResult['DROP_TOTAL_SIZE'] = .0;
			$this->arResult['DROP_TOTAL_COUNT'] = .0;

			$this->arResult['DROP_TRASHCAN'] = .0;
			$this->arResult['DROP_TRASHCAN_FORMAT'] = '';

			$this->arResult['DROP_UNNECESSARY_VERSION'] = .0;
			$this->arResult['DROP_UNNECESSARY_VERSION_FORMAT'] = '';

			$this->arResult['TOTAL_FILE_SIZE'] = 0;
			$this->arResult['TOTAL_FILE_SIZE_FORMAT'] = '';
			$this->arResult['TOTAL_FILE_COUNT'] = 0;

			if ($this->arResult['TrashCan']['FILE_COUNT'] > 0)
			{
				$this->arResult['DATA_COLLECTED'] = true;

				$this->arResult['DROP_TRASHCAN'] = $this->arResult['TrashCan']['FILE_SIZE'];
				$this->arResult['DROP_TRASHCAN_FORMAT'] = $this->arResult['TrashCan']['FILE_SIZE_FORMAT'];

				$this->arResult['TOTAL_FILE_COUNT'] += $this->arResult['TrashCan']['FILE_COUNT'];
				$this->arResult['TOTAL_FILE_SIZE'] += $this->arResult['TrashCan']['FILE_SIZE'];

				$this->arResult['DROP_TOTAL_COUNT'] += $this->arResult['TrashCan']['FILE_COUNT'];
				$this->arResult['DROP_TOTAL_SIZE'] += $this->arResult['TrashCan']['FILE_SIZE'];
			}

			if ($this->arResult['Storage']['FILE_COUNT'] > 0)
			{
				$this->arResult['DATA_COLLECTED'] = true;

				$this->arResult['DROP_UNNECESSARY_VERSION'] = $this->arResult['Storage']['UNNECESSARY_VERSION_SIZE'];
				$this->arResult['DROP_UNNECESSARY_VERSION_FORMAT'] = $this->arResult['Storage']['UNNECESSARY_VERSION_SIZE_FORMAT'];

				$this->arResult['TOTAL_FILE_COUNT'] += $this->arResult['Storage']['FILE_COUNT'];
				$this->arResult['TOTAL_FILE_SIZE'] += $this->arResult['Storage']['FILE_SIZE'];

				$this->arResult['DROP_TOTAL_COUNT'] += $this->arResult['Storage']['UNNECESSARY_VERSION_COUNT'];
				$this->arResult['DROP_TOTAL_SIZE'] += $this->arResult['Storage']['UNNECESSARY_VERSION_SIZE'];
			}
			if (count($this->arResult['MODULES']['LIST']) > 0)
			{
				$this->arResult['DATA_COLLECTED'] = true;
			}
			...
			if ($this->useDiskSizeAsTotalVolume())
			{
				if ($this->arResult['MODULES']['DISK_SIZE'] <= 0)
				{
			...
			{
				if ($this->arResult['MODULES']['FILE_SIZE'] <= 0)
				{
					$this->arResult['MODULES']['LIST'] = array();
				}
			}
			...
			if ($this->isOnlySpecificStorage() || !$this->isAdminMode())
			{
				$this->arResult['TOTAL_FILE_SIZE_FORMAT'] = \CFile::formatSize($this->arResult['TOTAL_FILE_SIZE']);
			}
			else
			{
				if ($this->useDiskSizeAsTotalVolume())
				{
					$this->arResult['TOTAL_FILE_COUNT'] = $this->arResult['MODULES']['DISK_COUNT'];
					$this->arResult['TOTAL_FILE_SIZE'] = $this->arResult['MODULES']['DISK_SIZE'];
				}
				else
				{
					$this->arResult['TOTAL_FILE_COUNT'] = $this->arResult['MODULES']['FILE_COUNT'];
					$this->arResult['TOTAL_FILE_SIZE'] = $this->arResult['MODULES']['FILE_SIZE'];
				}
				$this->arResult['TOTAL_FILE_SIZE_FORMAT'] = $this->arResult['MODULES']['FILE_SIZE_FORMAT'];
			}
			$this->arResult['DROP_TOTAL_SIZE_FORMAT'] = \CFile::formatSize($this->arResult['DROP_TOTAL_SIZE']);

			list($dropTotalSizeDigit, $dropTotalSizeUnits) = explode(' ', $this->arResult['DROP_TOTAL_SIZE_FORMAT']);
			$this->arResult['DROP_TOTAL_SIZE_DIGIT'] = $dropTotalSizeDigit;
			$this->arResult['DROP_TOTAL_SIZE_UNITS'] = $dropTotalSizeUnits;
		}
		catch (\Bitrix\Main\SystemException $exception)
		{
			$this->errorCollection->add(array(new Error($exception->getMessage(), $exception->getCode())));
	...
	protected function processActionDisks()
	{
		...
		try
		{
			...
			$indicatorStorage = $this->getIndicator(Volume\Storage\Storage::getIndicatorId());
			if ($this->isOnlySpecificStorage() || !$this->isAdminMode())
			{
				$indicatorStorage->addFilter('STORAGE_ID', $this->storageId);
			}
			$indicatorStorage->loadTotals();
			$this->arResult['TOTAL_FILE_SIZE'] += $indicatorStorage->getTotalSize();
			$this->arResult['TOTAL_FILE_COUNT'] += $indicatorStorage->getTotalCount();

			$indicatorTrashCan = $this->getIndicator(Volume\Storage\TrashCan::getIndicatorId());
			if ($this->isOnlySpecificStorage() || !$this->isAdminMode())
			{
				$indicatorTrashCan->addFilter('STORAGE_ID', $this->storageId);
			}
			$indicatorTrashCan->loadTotals();

			// total used space
			$this->arResult['TOTAL_FILE_SIZE'] += $indicatorTrashCan->getTotalSize();
			$this->arResult['TOTAL_FILE_COUNT'] += $indicatorTrashCan->getTotalCount();
			$this->arResult['TOTAL_FILE_SIZE_FORMAT'] = \CFile::formatSize($this->arResult['TOTAL_FILE_SIZE']);
	...
	protected function processActionStorage()
	{
		...
		try
		{
			...
			// total used space
			$this->arResult['TOTAL_FILE_SIZE'] = $this->arResult['Storage']['FILE_SIZE'] + $this->arResult['TrashCan']['FILE_SIZE'];
			$this->arResult['TOTAL_FILE_COUNT'] = $this->arResult['Storage']['FILE_COUNT'] + $this->arResult['TrashCan']['FILE_COUNT'];
			$this->arResult['TOTAL_FILE_SIZE_FORMAT'] = \CFile::formatSize($this->arResult['TOTAL_FILE_SIZE']);
	...
	private function loadMeasurementTotals($resId, $indicatorId, $filter = array())
	{
		...
		if ($indicator->getTotalCount() > 0)
		{
			...
			$this->arResult[$resId]['LIST'][$indicatorId] = array(
				...
				'FILE_SIZE' => $indicator->getTotalSize(),
				'FILE_SIZE_FORMAT' => \CFile::formatSize($indicator->getTotalSize()),
	...
	private function loadMeasurementResult($resId, $indicatorId, $filter = array(), $order = array(), $pagination = array(), $recalculatePercent = false)
	{
		...
		if ($indicator->getTotalCount() > 0 || $indicator instanceof \Bitrix\Disk\Volume\File)
		{
			...
			foreach ($result as $row)
			{
				...
				{
					$percent = (double)$row['FILE_SIZE'] * 100 / $totalSize;
	...
	private function loadModulesResult()
	{
		...
		if ($totals->getTotalSize() > 0)
		{
			...
			{
				$sizeField = 'DISK_SIZE';
				$totalSize = $res['FILE_SIZE'] = (double)$totals->getDiskSize();
				$totalCount = $res['FILE_COUNT'] = (double)$totals->getTotalVersion();
				$diskSize = $totalSize;
			...
			{
				$sizeField = 'FILE_SIZE';
				$totalSize = $res['FILE_SIZE'] = (double)$totals->getTotalSize();
				$totalCount = $res['FILE_COUNT'] = (double)$totals->getTotalCount();
			...
			foreach ($res['LIST'] as $indicatorModuleTypeId => $moduleMeasurement)
			{
				if ($indicatorModuleTypeId != $indicatorDiskTypeId)
				{
					...
					$moduleOtherSum += ((double)$moduleMeasurement['FILE_SIZE'] - (double)$moduleMeasurement['DISK_SIZE']);
			...
			foreach ($res['LIST'] as $indicatorModuleTypeId => $moduleMeasurement)
			{
				//$res['LIST'][$indicatorModuleTypeId]['FILE_SIZE'] = $moduleMeasurement["$sizeField"];
				//$res['LIST'][$indicatorModuleTypeId]['FILE_SIZE_FORMAT'] = $moduleMeasurement["{$sizeField}_FORMAT"];
	...
	public function getIndicatorResult(&$row)
	{
		if (!$row['_INDICATOR'] instanceof \Bitrix\Disk\Volume\IVolumeIndicator)
		{
			...
			if (
				...
				$row['ENTITY_TYPE'] === \Bitrix\Im\Disk\ProxyType\Im::className()
			)
			{
	...
	public function decorateStorageIcon(&$row, &$storage, $width = 50, $height = 50)
	{
		...
		switch ($entityType)
		{
			...
			case \Bitrix\Disk\ProxyType\Common::className():
			{
				$row['STYLE'] = 'Common';
				break;
			}

			case \Bitrix\Im\Disk\ProxyType\Im::className():
			{
				$row['STYLE'] = 'Im';
				break;
			}
	...
	public function getStorageMenuItems()
	{
		...
		{
			$actionMenu[] = $this->getMenuItemDiskSendNotification($this->arResult['Storage']['LIST'][0], $this->storage);
			$actionMenu[] = $this->getMenuItemDiskClearance($this->arResult['Storage']['LIST'][0], $this->storage);
		}
	...
	{
		$isImStorage = ($storage->getEntityType() === \Bitrix\Im\Disk\ProxyType\Im::className());
			...
			switch ($indicator::getIndicatorId())
			{
				...
				case \Bitrix\Disk\Volume\Storage\User::getIndicatorId():
				{
					...
					$action = array(
						...
						'onclick' => "BX.Disk.measureManager.openConfirm({".
									 " messageConfirm: BX.message('DISK_VOLUME_DELETE_STORAGE_SAFE_CLEAR_CONFIRM'), ".
									 " name: '".\CUtil::JSEscape($row['TITLE'])."', ".
									 " payload: 'callAction', ".
									 " action: '".self::ACTION_SETUP_CLEANER_JOB."', ".
									 " metric: '".self::METRIC_MARK_CERTAIN_DISK_CLEAN."', ".
									 " ".self::ACTION_DELETE_UNNECESSARY_VERSION.": 'Y',".
									 " filterIdsStorage: [".(int)$row['ID']."],".
									 " ".self::ACTION_EMPTY_TRASHCAN.": '$dropTrashcanFlag',".
									 " filterIdsTrashCan: [".(int)$trashcanFilterId."],".
									 " storageId: ".(int)$row['STORAGE_ID'].",".
									 " before: function(){BX.Disk.showActionModal({text: BX.message('DISK_VOLUME_SETUP_CLEANER'), showLoaderIcon: true, autoHide: false});}".
									 "});",
					);
					break;
				}
				...
				case \Bitrix\Disk\Volume\Storage\TrashCan::getIndicatorId():
				{
					$action = array(
						"text" => Loc::getMessage('DISK_VOLUME_ACTION_SAFE_CLEAR'),
						'onclick' => "BX.Disk.measureManager.openConfirm({".
									 " messageConfirm: BX.message('DISK_VOLUME_DELETE_TRASHCAN_SAFE_CLEAR_CONFIRM'), ".
									 " name: '".\CUtil::JSEscape($row['TITLE'])."', ".
									 " payload: 'callAction', ".
									 " action: '".self::ACTION_SETUP_CLEANER_JOB."', ".
									 " metric: '".self::METRIC_MARK_CERTAIN_DISK_CLEAN."', ".
			...
			switch ($entityType)
			{
				...
				case \Bitrix\Disk\ProxyType\Group::className():
				{
					$action = array(
						...
						'onclick' => "BX.Disk.measureManager.callAction({".
	...
	public function getFolderActionMenu(&$row, &$storage)
	{
		...
		try
		{
			...
			if ($fragment->getIndicatorType() == \Bitrix\Disk\Volume\Folder::className())
			{
				$isRootFolder = ($row['FOLDER_ID'] == $storage->getRootObjectId());

				if ($this->arResult["ADMIN_MODE"])
				{
					$actions[] = $this->getMenuItemDiskSendNotification($row, $storage);
				}
				...
				if ($fragment->getUnnecessaryVersionCount() > 0)
				{
					...
					$actions[] = array(
						...
						'onclick' => "BX.Disk.measureManager.openConfirm({".
									 " messageConfirm: BX.message('{$messageId}'), ".
									 " name: '".\CUtil::JSEscape($row['TITLE'])."', ".
									 " payload: 'callAction', ".
									 " action: '".self::ACTION_SETUP_CLEANER_JOB."', ".
				...
				if ($this->isAllowDeleteFolder($row, $storage))
				{
					$actions[] = array(
						"text" => Loc::getMessage('DISK_VOLUME_ACTION_DELETE_FOLDER'),
						'onclick' => "BX.Disk.measureManager.openConfirm({".
									 " messageConfirm: BX.message('DISK_VOLUME_DELETE_FOLDER_CONFIRM'), ".
									 " name: '".\CUtil::JSEscape($row['TITLE'])."', ".
									 " payload: 'callAction', ".
									 " action: '".self::ACTION_SETUP_CLEANER_JOB."', ".
									 " metric: '".self::METRIC_MARK_CERTAIN_FOLDER_CLEAN."', ".
	...
	public function getFileActionMenu(&$row, &$storage)
	{
		...
		try
		{
			...
			if ($fragment->getIndicatorType() == \Bitrix\Disk\Volume\File::className())
			{
				if ($fragment->getUnnecessaryVersionCount() > 0)
				{
					$actions[] = array(
						"text" => Loc::getMessage('DISK_VOLUME_ACTION_DELETE_UNNECESSARY_VERSION'),
						'onclick' => "BX.Disk.measureManager.openConfirm({".
									 " messageConfirm: BX.message('DISK_VOLUME_DELETE_FILE_UNNECESSARY_VERSION_CONFIRM'), ".
				...
				$actions[] = array(
					...
					'onclick' => "BX.Disk.measureManager.openConfirm({".
								 " messageConfirm: BX.message('DISK_VOLUME_DELETE_FILE_CONFIRM'), ".
	 ...
	 * @return array
	 */
	public function getDeletedFileActionMenu(&$row, &$storage)
	{
		$actions = array();
	 ...
	 *
	 * @param array $row Result row.
	 * @param \Bitrix\Disk\Storage $storage Storage contains folder.
	 *
	 * @return boolean
	 * @throws \Bitrix\Main\SystemException
	 */
	public function isAllowDeleteFolder(&$row, &$storage)
	{
		$allowDrop = true;
