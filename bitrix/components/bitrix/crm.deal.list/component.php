...
if ($arParams['IS_RECURRING'] === 'Y')
{
	$arResult['HEADERS'] = array_merge(
		...
		array(
			...
			array('id' => 'CRM_DEAL_RECURRING_COUNTER_REPEAT', 'name' => GetMessage('CRM_COLUMN_COUNTER_REPEAT'), 'sort' => 'counter_repeat', 'default' => true, 'editable' => false),
			array('id' => 'CRM_DEAL_RECURRING_NEXT_EXECUTION', 'name' => GetMessage('CRM_COLUMN_NEXT_EXECUTION'), 'sort' => 'next_execution', 'default' => true, 'editable' => false),
...
{
	$waitings = \Bitrix\Crm\Pseudoactivity\WaitEntry::getRecentIDsByOwner(CCrmOwnerType::Deal, $aclivitylessItems);
	foreach($waitings as $waiting)
	{
		$entityID = (int)$waiting['OWNER_ID'];
		$waitingID = (int)$waiting['ID'];
		if(isset($arResult['DEAL'][$entityID]))
		{
			$arResult['DEAL'][$entityID]['~WAITING_ID'] = $waitingID;
		}
	}
}
