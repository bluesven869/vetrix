function CrmWebForm(params)
{
	...
	this.initFrameParameters = function()
	{
		...
		if(frameParameters.options)
		{
			...
			{
				if(frameParameters.options['css']['content'])
				{
					this.addCssContent(frameParameters.options['css']['content']);
				}

				if(frameParameters.options['css']['file'])
				{
					if(Array.isArray(frameParameters.options['css']['file']))
					{
						frameParameters.options['css']['file'].forEach(function(file){
							BX.loadCSS(file);
						})
					}
					else
					{
						BX.loadCSS(frameParameters.options['css']['file']);
					}
				}
			}
		}
