...
if($action == 'DELETE')
{
	...
	if (!empty($_REQUEST['IS_SPAM']) && $_REQUEST['IS_SPAM'] == 'Y' && \CModule::includeModule('mail'))
	{
...
elseif($action == 'SAVE_EMAIL')
{
	...
	if (\CCrmContentType::Html == $contentType)
	{
		if (isset($rawData['message']))
		{
			...
			\CCrmActivity::addEmailSignature($message, $contentType);

			$message = preg_replace('/<!--.*?-->/is', '', $message);
			$message = preg_replace('/<script[^>]*>.*?<\/script>/is', '', $message);
			$message = preg_replace('/<title[^>]*>.*?<\/title>/is', '', $message);
			...
			$messageHtml = $message = $sanitizer->sanitizeHtml($message);
		}
	}
	else
	{
		if (isset($data['message']))
		{
			...
			if (\CCrmContentType::PlainText == $contentType)
			{
				$messageHtml = sprintf(
					...
					preg_replace('/[\r\n]+/'.BX_UTF_PCRE_MODIFIER, '<br>', htmlspecialcharsbx($message))
				);
			}
	...
	{
		$checkInlineFiles = \CCrmContentType::Html == $arFields['DESCRIPTION_TYPE'];
	...
	if (!empty($_REQUEST['save_as_template']) && \CCrmOwnerType::isDefined($_REQUEST['save_as_template']))
	{
		$templateFields = array(
			...
			'ENTITY_TYPE_ID' => (int) $_REQUEST['save_as_template'],
			'EMAIL_FROM'     => $from,
			'SUBJECT'        => $subject,
	...
	foreach ($arRawFiles as $key => $item)
	{
		$contentId = sprintf(
			'bxacid.%s@%s',
			hash('crc32b', $item['external_id'].$item['tmp_name']),
			$hostname
		);
...
elseif($action == 'SEARCH_COMMUNICATIONS')
{
	...
	{
		if(!Bitrix\Crm\Authorization\Authorization::checkReadPermission(
