...
if(!function_exists('__CrmImportCompanyAddressesToRequisite'))
{
	function __CrmImportCompanyAddressesToRequisite($companyFields, $sourceFields, $dupCtrlType, $impAddrPresetId)
	{
		...
		if (!empty($addresses))
		{
			...
			try
			{
				$rqImportResult = $requisite->importEntityRequisite(
...
$paramValue = Bitrix\Crm\Settings\CompanySettings::getCurrent()->areOutmodedRequisitesEnabled() ? 'N' : 'Y';
...
if (isset($_POST['IMPORT_ADDR_PRESET']))
	...
	$defPresetId = EntityRequisite::getDefaultPresetId(CCrmOwnerType::Company);
