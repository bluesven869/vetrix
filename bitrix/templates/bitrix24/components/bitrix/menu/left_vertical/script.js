...
BX.Bitrix24.LeftMenuClass = {
	...
	showSettingsPopup: function (event)
	{
		...
		//custom preset
		if (this.isCustomPresetAvailable)
		{
			if (this.isAdmin || this.customPresetExists)
			{
				menuItems.push({
					text: '<span>'+BX.message('MENU_CUSTOM_PRESET_SEPARATOR')+'</span>',
					className: "menu-popup-item menu-buttons-submenu-separator menu-buttons-hidden-label"
				});
			}
			...
			if (this.isAdmin)
			{
				...
				if (this.customPresetExists)
				{
					menuItems.push({
						text: BX.message("MENU_DELETE_CUSTOM_PRESET"),
						className: "menu-popup-no-icon",
						onclick: function ()
						{
							this.popupWindow.close();
							self.deleteCustomPreset();
						}
					});
				}
			}
			...
			{
				menuItems.push({
					text: BX.message("MENU_SET_CUSTOM_PRESET"),
					className: "menu-popup-no-icon",
					onclick: function ()
					{
						this.popupWindow.close();
						self.setCustomPreset();
					}
				});
			}
		}
	...
	showCustomPresetPopup: function ()
	{
		...
		var menuItemsSort = {"show": showMenuItems, "hide": hideMenuItems};
	...
	deleteCustomPreset: function ()
	{
		BX.PopupWindowManager.create("menu-custom-preset-delete-popup", null, {
			closeIcon: true,
			contentColor : "white",
			contentNoPaddings : true,
			content: "<div style='padding: 10px;'>" + BX.message("MENU_DELETE_CUSTOM_PRESET_CONFIRM") + "</div>",
			titleBar: BX.message("MENU_DELETE_CUSTOM_PRESET"),
			buttons: [
				(button = new BX.PopupWindowButton({
					text: "OK",
					className: "popup-window-button-create",
