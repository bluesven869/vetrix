	...
	switch ($_POST["action"])
	{
		...
		*/
		...
		case "delete_custom_preset":
			...
			COption::RemoveOption("intranet", "left_menu_preset", $siteID);
			COption::RemoveOption("intranet", "left_menu_first_page", $siteID);

			break;

		case "set_custom_preset":
			CUserOptions::DeleteOption("intranet", "left_menu_first_page_".$siteID);
			CUserOptions::DeleteOption("intranet", "left_menu_preset");
			CUserOptions::DeleteOption("intranet", "left_menu_sorted_items_".$siteID);

			//check duplicates in self and custom items. Delete self item if it's duplicate
			$selfItems = CUserOptions::GetOption("intranet", "left_menu_self_items_".$siteID);
			$customItems = COption::GetOptionString("intranet", "left_menu_custom_preset_items", "");
			if (empty($customItems) || empty($selfItems))
				break;
			...
			if (is_array($selfItems))
			{
				foreach ($selfItems as $key => $item)
				{
					if (in_array($item["ID"], $customItemsId))
					{
						unset($selfItems[$key]);
					}
				}
