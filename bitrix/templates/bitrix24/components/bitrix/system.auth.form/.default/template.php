...
<?
//Remove this code after 11.11.2017
...
?>
	...
	BX.ready(function() {
		...
		var left = BX("user-name") ? node.offsetWidth - 4 : 85;
					...
					click: function(event) {
						if (event.target.tagName === "SPAN")
						{
							...
							else
							{
								if (spotlight.popup)
								{
									spotlight.popup.destroy();
								}

								BX.remove(spotlight.container);
							}

							BX.Intranet.Bitrix24.ThemePicker.Singleton.ajax({ action: "hide-hint" });
							BX.Intranet.Bitrix24.ThemePicker.Singleton.showDialog();
						}
					}
				},
				html: "<?=GetMessageJS(
							"AUTH_THEME_DIALOG_HINT",
							array("<span>" => '<span style="border-bottom: 1px dashed; cursor: pointer;">')
						)?>"
			})
		});
		...
		var onresize = function() {
			...
			if (spotlight.container && spotlight.renderTo)
			{
				var pos = BX.pos(spotlight.renderTo);
		...
		BX.addCustomEvent(spotlight, "spotLightOk", function() {
			...
			BX.unbind(window, "resize", onresize);

			setTimeout(function() {
				showUserMenu();

				var menu = BX.PopupMenu.getMenuById("user-menu");
				if (!menu || !menu.getMenuItem("theme-picker"))
				{
					return;
				}

				var item = menu.getMenuItem("theme-picker").getLayout().item;
				item.style.fontWeight = "bold";
				item.style.backgroundColor = "#fff4cb";
