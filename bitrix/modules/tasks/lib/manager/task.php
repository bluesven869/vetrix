...
final class Task extends \Bitrix\Tasks\Manager
{
	...
	public static function getList($userId, array $listParameters = array(), array $parameters = array())
	{
		...
		if(array_key_exists('SORTING', $listParameters[ 'order' ])
		...
		foreach ($arFields as $field)
		{
			switch ($field)
			{
				...
				case 'CHECKLIST':
					...
					break;
				case 'RESPONSIBLE':
					$userInfo = \Bitrix\Tasks\UI\Task::getUserInfo($arTask[ 'RESPONSIBLE_ID' ]);
					$index[] = (!empty($userInfo[ 'NAME' ]) ? $userInfo[ 'NAME' ] : '');
					break;
				case 'ORIGINATOR':
					$userInfo = \Bitrix\Tasks\UI\Task::getUserInfo($arTask[ 'CREATED_BY' ]);
					$index[] = (!empty($userInfo[ 'NAME' ]) ? $userInfo[ 'NAME' ] : '');
					break;
				case 'AUDITORS':
					if(array_key_exists('AUDITORS', $arTask))
					{
						...
						{
							$userInfo = \Bitrix\Tasks\UI\Task::getUserInfo($userId);
							$index[] = (!empty($userInfo[ 'NAME' ]) ? $userInfo[ 'NAME' ] : '');
						}
					}
					break;
...
//                    break;
				case 'CRM':
					$uf = is_object($arTask[ 'UF_CRM_TASK' ]) ? $arTask[ 'UF_CRM_TASK' ]->toArray() : (array)$arTask[ 'UF_CRM_TASK' ];
					foreach ($uf as $item)
					{
