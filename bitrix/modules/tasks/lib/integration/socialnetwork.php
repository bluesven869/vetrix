...
abstract class SocialNetwork extends \Bitrix\Tasks\Integration
{
	...
	public static function getLogDestination($context = 'TASKS', array $parameters = array())
	{
		...
		\CSocNetLogDestination::fillLastDestination(
			...
			$destination["LAST"],
			array(
				"EMAILS" => \Bitrix\Main\ModuleManager::isModuleInstalled("mail")
			)
		);
		...
		else
		{
			...
			$limitReached = false;
			$destination["SONETGROUPS"] = \CSocNetLogDestination::getSocnetGroup(array_merge($destinationParams, array("ALL" => "Y", "GROUP_CLOSED" => "N", "features" => array("tasks", array("create_tasks")))), $limitReached);
