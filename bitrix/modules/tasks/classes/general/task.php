		...
		if ($this->CheckFields($arFields, false, $effectiveUserId))
		{
			...
			if ($USER_FIELD_MANAGER->CheckFields("TASKS_TASK", 0, $arFields, $effectiveUserId))
			{
				...
				if ($ID)
				{
					...
					$DB->Query('UPDATE b_tasks SET SEARCH_INDEX = \''.
			...
			switch ($key)
			{
				...
				case "STATUS":
					$arSqlSearch[] = CTasks::FilterCreate("
						CASE
							WHEN
								".$sAliasPrefix."T.DEADLINE < DATE_ADD(".$DB->CurrentTimeFunction().", INTERVAL ".\CTaskCountersProcessor::DEADLINE_TIME." SECOND)
	...
	public static function GetList($arOrder=array(), $arFilter=array(), $arSelect = array(), $arParams = array(), array $arGroup = array())
	{
		...
		$arFields = array(
			...
			"STATUS" => "
				CASE
					WHEN
						T.DEADLINE < DATE_ADD(".$DB->CurrentTimeFunction().", INTERVAL ".\CTaskCountersProcessor::DEADLINE_TIME." SECOND)
