...
class LeadConverter extends EntityConverter
{
			...
			if(is_array($presentFields))
			{
				...
				if(!empty($fields))
				{
					...
					if($entity->Update($this->entityID, $fields, true, true, array('REGISTER_SONET_EVENT' => true)))
					{
						...
						if(isset($this->resultData[\CCrmOwnerType::CompanyName]))
						{
							$companyID = $this->resultData[\CCrmOwnerType::CompanyName];
						}

						$contactID = 0;
						if(isset($this->resultData[\CCrmOwnerType::ContactName]))
						{
							$contactID = $this->resultData[\CCrmOwnerType::ContactName];
						}
				...
				if(isset($this->resultData[\CCrmOwnerType::CompanyName]))
				{
					$this->attachEntity(\CCrmOwnerType::Company, $this->resultData[\CCrmOwnerType::CompanyName]);
				}
				if(isset($this->resultData[\CCrmOwnerType::ContactName]))
				{
					$this->attachEntity(\CCrmOwnerType::Contact, $this->resultData[\CCrmOwnerType::ContactName]);
				}
				if(isset($this->resultData[\CCrmOwnerType::DealName]))
				{
					$this->attachEntity(\CCrmOwnerType::Deal, $this->resultData[\CCrmOwnerType::DealName]);
				}
				//endregion
			}
	...
	{
		Crm\Timeline\CreationEntry::attach(\CCrmOwnerType::Lead, $this->entityID, $entityTypeID, $entityID);
		Crm\Timeline\CommentEntry::attach(\CCrmOwnerType::Lead, $this->entityID, $entityTypeID, $entityID);
		Crm\Timeline\ActivityEntry::attach(\CCrmOwnerType::Lead, $this->entityID, $entityTypeID, $entityID);
