	...
	public function findRequisiteDuplicates(\Bitrix\Crm\EntityAdapter $adapter, DuplicateSearchParams $params)
	{
		...
		foreach ($requsiiteDupFieldsMap as $countryId => $fields)
		{
			foreach ($fields as $fieldName)
			{
				...
				{
					$requsiiteFieldGroups[$groupId] = array(
						'countryId' => $countryId,
						'fieldName' => $fieldName,
						'values' => array()
					);
		...
		{
			if (strlen($fieldName) > 3 && 'RQ.' === substr($fieldName, 0, 3)
				&& !in_array(substr($fieldName, 3), array_keys($requsiiteFieldGroups))
			)
			{
		...
		if (!empty($requsiiteFieldGroups))
		{
			...
			foreach ($allRequisites as $requisiteFields)
			{
				...
				if (isset($requisiteFields['ID']) && $countryId > 0)
				{
					...
					{
						$groupId = $fieldName.'|'.$countryId;
						if (isset($requsiiteFieldGroups[$groupId]))
							$requsiiteFieldGroups[$groupId]['values'][] = $value;
					}
				}
			}
			...
			$criterions = array();
			foreach($requsiiteFieldGroups as $requsiiteFieldGroup)
			{
