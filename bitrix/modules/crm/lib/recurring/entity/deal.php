		...
		try
		{
			...
			foreach ($idListChunks as $idList)
			{
				...
				while ($deal = $dealsData->Fetch())
				{
					...
					$reCalculate = false;
					$resultDeal = $newDeal->Add($deal, $reCalculate, array('DISABLE_TIMELINE_CREATION' => 'Y'));

					if ($resultDeal)
					{
						if (!empty($products[$recurringDealId]))
						{
							$newDeal::SaveProductRows($resultDeal, $products[$recurringDealId], true, true, false);
						}

						$productRowSettings = \CCrmProductRow::LoadSettings('D', $recurringDealId);
						if (!empty($productRowSettings))
							\CCrmProductRow::SaveSettings('D', $resultDeal, $productRowSettings);

						$newDealIds[] = $resultDeal;
