...
BX.CRM.Kanban.Item.prototype = {
	...
	createLayout: function()
	{
		...
		if (gridData.showActivity)
		{
			...
			this.activityEmpty = BX.create("span", {
				props: {
					className: "crm-kanban-item-activity-empty"
				},
				events: {
					mouseover: BX.delegate(function()
					{
						this.showTooltip(
							BX.message("CRM_KANBAN_ACTIVITY_LETSGO_" + gridData.entityType),
							BX.proxy_context
						);
					}, this),
	...
	showCurrentPlan: function()
	{
		...
		BX.addClass(this.container, "crm-kanban-item-hover");
	...
	switchPlanner: function()
	{
		...
		if (data.activityProgress > 0)
		{
			...
			this.activityExist.textContent = BX.message("CRM_KANBAN_ACTIVITY_PLAN") +
											": " + data.activityProgress;
		}
		else
		{
			this.switchVisible(this.activityExist, false);
			this.switchVisible(this.activityEmpty, columnData.type !== "WIN");
			this.activityEmpty.textContent = BX.message("CRM_KANBAN_ACTIVITY_PLAN");
		}
	},
	 ...
	 * @returns {void}
	 */
	showTooltip: function(message, context)
	{
		this.popupTooltip = new BX.PopupWindow(
			"kanban_tooltip",
			BX.proxy_context,
			{
				className: "crm-kanban-without-tooltip",
				offsetLeft: 14,
				darkMode: true,
				closeByEsc: true,
				angle : true,
	...
	addHoverClass: function (itemBlock)
	{
		...
		{
			itemBlock.classList.add("crm-kanban-item-event");
		}, 400);
	},
	...
	removeHoverClass: function (itemBlock)
	{
		...
		itemBlock.classList.remove("crm-kanban-item-event");
	},
