...
class CAllCrmActivity
{
	...
	public static function AttachBinding($srcOwnerTypeID, $srcOwnerID, $targOwnerTypeID, $targOwnerID)
	{
		...
		$dbResult = $connection->query(
			"SELECT ACTIVITY_ID FROM b_crm_act_bind WHERE OWNER_TYPE_ID = {$srcOwnerTypeID} AND OWNER_ID = {$srcOwnerID}"
		);

		$itemIDs = array();
		while($fields = $dbResult->fetch())
		{
			$itemIDs[] = $fields['ACTIVITY_ID'];
		}

		foreach($itemIDs as $itemID)
		{
			$dbResult = $connection->query(
				"SELECT 1 FROM b_crm_act_bind WHERE ACTIVITY_ID = {$itemID} AND OWNER_TYPE_ID = {$targOwnerTypeID} AND OWNER_ID = {$targOwnerID}"
			);
			if($dbResult->fetch() === false)
			{
				$connection->queryExecute(
					"INSERT INTO b_crm_act_bind(ACTIVITY_ID, OWNER_TYPE_ID, OWNER_ID) SELECT {$itemID}, {$targOwnerTypeID}, {$targOwnerID}"
				);
			}
		}
	}
	...
	public static function PrepareDescriptionFields(array &$fields)
	{
		...
		elseif($descriptionType === \CCrmContentType::BBCode)
		{
			...
			$fields['DESCRIPTION_HTML'] = $parser->convertText($description);
