...
class CAllCrmDeal
{
	...
	public function Update($ID, array &$arFields, $bCompare = true, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			elseif($enableCloseDateSync && self::GetStageSemantics($arRow['STAGE_ID'], $categoryID) !== 'process' && isset($arFields['CLOSEDATE']))
			{
				unset($arFields['CLOSEDATE']);
			}
			elseif(isset($arFields['CLOSEDATE']))
	...
	public function Delete($ID, $arOptions = array())
	{
		...
		$dbResult = \CCrmDeal::GetListEx(
			...
			false,
			false,
			array('TITLE', 'LEAD_ID', 'CATEGORY_ID', 'ASSIGNED_BY_ID', 'IS_RECURRING')
		);
	...
	public static function RefreshAccountingData(array $IDs)
	{
		...
		while($fields = $dbResult->Fetch())
		{
			...
			$exchRate = isset($fields['EXCH_RATE'])
				? (double)$fields['EXCH_RATE'] : -1;

			$currentExchRate = CCrmCurrency::GetExchangeRate($currencyID);
			if($currentExchRate === $exchRate)
			{
				continue;
			}

			$updateFields = array('CURRENCY_ID' => $currencyID, 'EXCH_RATE' => $currentExchRate);
			$entity->Update($ID, $updateFields, false, false, array('REGISTER_SONET_EVENT' => false));
		}
	}
