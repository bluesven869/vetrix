...
class DiskVolumeController extends \Bitrix\Disk\Internals\Controller
{
	...
	protected function processActionReloadTotals()
	{
		...
		{
			if (
				$this->component->isOnlySpecificStorage() ||
				!$this->component->isAdminMode() ||
				($this->component->isAdminMode() && $this->storageId > 0)
			)
			{
				/** @var \Bitrix\Disk\Volume\IVolumeIndicator $trashCanIndicator */
				$trashCanIndicator = $this->component->getIndicator(Volume\Storage\TrashCan::className());
				...
				/** @var \Bitrix\Disk\Volume\IVolumeIndicator $trashCanIndicator */
				$storageIndicator = $this->component->getIndicator(Volume\Storage\Storage::className());
				...
				$answer['DISK_SPACE'] = Volume\Storage\Storage::getAvailableSpace($this->storage);
			}
			elseif ($this->component->isAdminMode())
			{
				/** @var \Bitrix\Disk\Volume\IVolumeIndicator $trashCanIndicator */
				$trashCanIndicator = $this->component->getIndicator(Volume\Storage\TrashCan::className());
				...
				if ($trashCanIndicator->getTotalCount() > 0)
				{
					if ($this->component->useDiskSizeAsTotalVolume())
					{
				...
				// full disk limit
				$answer['DISK_SPACE'] = Volume\Storage\Storage::getAvailableSpace();
			}

			if ($answer['DISK_SPACE'] > 0)
			{
				$answer['DISK_SPACE_FORMAT'] = \CFile::formatSize($answer['DISK_SPACE']);
			}

			$answer['DROP_TOTAL_SIZE'] = .0;
			$answer['DROP_TOTAL_COUNT'] = .0;

			//$answer['DROP_TRASHCAN'] = .0;
			//$answer['DROP_TRASHCAN_FORMAT'] = '';

			//$answer['DROP_UNNECESSARY_VERSION'] = .0;
			//$answer['DROP_UNNECESSARY_VERSION_FORMAT'] = '';

			$answer['TOTAL_FILE_SIZE'] = 0;
			$answer['TOTAL_FILE_SIZE_FORMAT'] = \CFile::formatSize(0);
			$answer['TOTAL_FILE_COUNT'] = 0;
			$answer['DROP_UNNECESSARY_VERSION'] = 0;
			$answer['DROP_UNNECESSARY_VERSION_FORMAT'] = \CFile::formatSize(0);
			$answer['DROP_TRASHCAN'] = 0;
			$answer['DROP_TRASHCAN_FORMAT'] = \CFile::formatSize(0);

			// add other to totals
			if (
				$this->component->isAdminMode() &&
				!$this->component->useDiskSizeAsTotalVolume() &&
				!$this->component->isOnlySpecificStorage() &&
				!($this->component->isAdminMode() && $this->storageId > 0)
			)
			{
				$totals = $this->component->getIndicator(Volume\Bfile::className());
				$totals->loadTotals();
				if ($totals->getTotalSize() > 0)
				{
					$answer['TOTAL_FILE_SIZE'] += ((double)$totals->getTotalSize() - (double)$totals->getDiskSize());
					$answer['TOTAL_FILE_COUNT'] += ((double)$totals->getTotalCount() - (double)$totals->getTotalVersion());
				}
			}

			if ($trashCanInfo['FILE_COUNT'] > 0)
			{
				$answer['DROP_TRASHCAN'] = $trashCanInfo['FILE_SIZE'];
				$answer['DROP_TRASHCAN_FORMAT'] = $trashCanInfo['FILE_SIZE_FORMAT'];

				$answer['TOTAL_FILE_COUNT'] += $trashCanInfo['FILE_COUNT'];
				$answer['TOTAL_FILE_SIZE'] += $trashCanInfo['FILE_SIZE'];

				$answer['DROP_TOTAL_COUNT'] += $trashCanInfo['FILE_COUNT'];
				$answer['DROP_TOTAL_SIZE'] += $trashCanInfo['FILE_SIZE'];
			}
			if ($storageInfo['FILE_COUNT'] > 0)
			{
				$answer['DROP_UNNECESSARY_VERSION'] = $storageInfo['UNNECESSARY_VERSION_SIZE'];
				$answer['DROP_UNNECESSARY_VERSION_FORMAT'] = $storageInfo['UNNECESSARY_VERSION_SIZE_FORMAT'];

				$answer['TOTAL_FILE_COUNT'] += $storageInfo['FILE_COUNT'];
				$answer['TOTAL_FILE_SIZE'] += $storageInfo['FILE_SIZE'];

				$answer['DROP_TOTAL_COUNT'] += $storageInfo['UNNECESSARY_VERSION_COUNT'];
				$answer['DROP_TOTAL_SIZE'] += $storageInfo['UNNECESSARY_VERSION_SIZE'];
			}


			$answer['TOTAL_FILE_SIZE_FORMAT'] = \CFile::formatSize($answer['TOTAL_FILE_SIZE']);
			$answer['DROP_TOTAL_SIZE_FORMAT'] = \CFile::formatSize($answer['DROP_TOTAL_SIZE']);

			list($dropTotalSizeDigit, $dropTotalSizeUnits) = explode(' ', $answer['DROP_TOTAL_SIZE_FORMAT']);
			$answer['DROP_TOTAL_SIZE_DIGIT'] = $dropTotalSizeDigit;
			$answer['DROP_TOTAL_SIZE_UNITS'] = $dropTotalSizeUnits;
		...
		if ($this->queueLength > 0 && $this->queueStep > 0)
		{
			...
			else
			{
				if ($this->queueStep >= $this->queueLength)
				{
					$this->component->clearQueueStep();
					$this->sendJsonSuccessResponse(array(
						//'message' => Loc::getMessage('DISK_VOLUME_DATA_COLLECTED'),
	...
	protected function processActionMeasureFolder()
	{
		...
		if ($subTaskDone)
		{
			...
			if ($this->queueLength > 0 && $this->queueStep > 0)
			{
				...
				{
					$this->component->clearQueueStep();
					//$responseParams['message'] = Loc::getMessage('DISK_VOLUME_DATA_COLLECTED');
					$responseParams['queueStep'] = $this->queueStep;
