...
class Crm extends Volume\Module\Module implements Volume\IVolumeIndicatorLink, Volume\IDeleteConstraint, Volume\IVolumeTimeLimit
{
	...
	public function measure($collectData = array())
	{
		...
		switch($stepId)
		{
			...
			case 'ActElemFile':
			{
				// analise b_crm_act_elem with STORAGE_TYPE_ID = \Bitrix\Crm\Integration\StorageType::Disk
				$querySql = "
					INSERT INTO {$tableName}
					(
						INDICATOR_TYPE,
	...
	public function getFolderList($storage)
	{
		if ($storage instanceof \Bitrix\Disk\Storage && count($this->folderList[$storage->getId()]) == 0)
		{
			...
			$typeFolderXmlId = $this->getSpecialFolderXmlId();
	 ...
	 * @return string[]
	 */
	public function getSpecialFolderXmlId()
	{
