...
class Storage extends Volume\Base implements Volume\IVolumeIndicatorStorage, Volume\IVolumeIndicatorLink
{
	 ...
	 * @return $this
	 */
	public function measure($collectData = array(self::DISK_FILE, self::UNNECESSARY_VERSION))
	{
	...
	public function recalculatePercent($totalSizeIndicator = '\\Bitrix\\Disk\\Volume\\Module\\Disk', $excludeSizeIndicator = '')
	{
		...
		$total = (double)$totalSizeIndicator->getTotalSize();
		...
		if ($excludeSizeIndicator instanceof Volume\IVolumeIndicator)
		{
			...
			$total -= (double)$excludeSizeIndicator->getTotalSize();
		}
			...
			$sql = "
				UPDATE 
					...
					(
						...
						FROM
						(
							SELECT ID, STORAGE_ID, FILE_SIZE
							FROM 
								{$tableName} 
							WHERE 
								OWNER_ID = {$ownerId}
	...
	public static function getUrl(Volume\Fragment $fragment)
	{
		...
		if ($storage->getEntityType() == \Bitrix\Im\Disk\ProxyType\Im::className())
		{
