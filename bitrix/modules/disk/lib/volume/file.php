...
class File extends Volume\Base implements Volume\IVolumeIndicatorParent, Volume\IVolumeIndicatorLink
{
	 ...
	 * @return DB\Result
	 */
	public function getMeasurementResult($collectedData = array(self::DISK_FILE, self::ATTACHED_OBJECT, self::EXTERNAL_LINK, self::UNNECESSARY_VERSION))
	{
		$connection = Application::getConnection();
		...
		//if ($orderKey == 'USING_COUNT') $fromSql .= $buildQueryAttached($whereSql);
		//if ($orderKey == 'USING_COUNT') $fromSql .= $buildQueryExternal($whereSql);
		//if ($orderKey == 'USING_COUNT') $fromSql .= $buildQuerySharing($whereSql);
	...
	public static function getParents(Volume\Fragment $fragment)
	{
		...
		// Im
		if ($fragment->getEntityType() == \Bitrix\Im\Disk\ProxyType\Im::className())
		{
