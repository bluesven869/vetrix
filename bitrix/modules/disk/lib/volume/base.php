...
abstract class Base implements \Bitrix\Disk\Volume\IVolumeIndicator, IErrorable
{
	...
	/** @var double */
	protected $diskSize = -1;

	/** @var double */
	protected $diskCount = -1;

	/** @var double */
	protected $totalVersion = -1;

	/** @var double */
	protected $totalAttached = -1;

	/** @var double */
	protected $totalLink = -1;

	/** @var double */
	protected $totalSharing = -1;

	/** @var double */
	protected $unnecessaryVersionSize = -1;

	/** @var double */
	protected $unnecessaryVersionCount = -1;
	...
	/** @var array Delete constraint list. */
	public static $constraintList = array();
	 ...
	 */
	public function measure($collectData = array(self::REGULAR_FILE, self::DISK_FILE))
	{
	...
	public function getMeasurementResult($collectedData = array())
	{
		...
		$parameter = array(
			'runtime' => array(
				//new Entity\ExpressionField('PERCENT', 'ROUND(FILE_SIZE * 100 / '.$this->getTotalSize().', 1)'),
	...
	public function recalculatePercent($totalSizeIndicator = null, $excludeSizeIndicator = null)
	{
		if ($totalSizeIndicator instanceof Volume\IVolumeIndicator)
		{
			...
			$total = $totalSizeIndicator->getTotalSize();
		}
		else
		{
			$this->loadTotals();
	 ...
	 * @return array
	 */
	final public static function listConstraint()
	{
		if (empty(self::$constraintList))
		{
			self::loadListIndicator();
		}
			...
			if ($entry->isFile() && preg_match("/^(.+)\.php$/i", $entry->getName(), $parts))
			{
				...
				try
				{
					...
					)
					{
						...
						{
							self::$constraintList[$class::getIndicatorId()] = $class::className();
						}
					}
				}
