...
class RightsManager implements IErrorable
{
	...
	{
		$negaviteRights = array();
		foreach($rights as $right)
		{
			if(!empty($right['NEGATIVE']))
			{
				$negaviteRights[$right['TASK_ID'] . '-' . $right['ACCESS_CODE']] = true;
			}
		}
		unset($right);
		if($negaviteRights)
		{
			...
			{
				if(empty($right['NEGATIVE']) && isset($negaviteRights[$right['TASK_ID'] . '-' . $right['ACCESS_CODE']]))
				{
					unset($negaviteRights[$right['TASK_ID'] . '-' . $right['ACCESS_CODE']]);
				}
			}
			unset($right);
		}

		return count($negaviteRights) == 0;
	}
	...
	{
		$positiveRights = $negaviteRights = array();
		foreach($rights as $right)
		{
			if(!empty($right['NEGATIVE']))
			{
				$negaviteRights[$right['TASK_ID'] . '-' . $right['ACCESS_CODE']] = $right;
			}
			else
			{
		...
		return array_merge($positiveRights, array_values($negaviteRights));
	}
