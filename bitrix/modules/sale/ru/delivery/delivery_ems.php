...
class CDeliveryEMS
{
	...
	function __GetLocation($location)
	{
		...
		if ($arLocation["IS_RUSSIAN"] == 'Y')
		{
			...
			if(empty($arLocation['EMS_ID']) && (!empty($arLocation['REGION_NAME_ORIG']) || !empty($arLocation['REGION_SHORT_NAME']) || !empty($arLocation['REGION_NAME_LANG']) || !empty($arLocation['REGION_NAME'])))
			{
				...
				if($arLocation['REGION_NAME_ORIG'] == '—‡ı‡ /ﬂÍÛÚËˇ/ –ÂÒÔ' || $arLocation['REGION_NAME_ORIG'] == '–ÂÒÔÛ·ÎËÍ‡ —‡ı‡ (ﬂÍÛÚËˇ)')
					$arLocation['REGION_NAME_ORIG']  = '—¿’¿ (ﬂ ”“»ﬂ) –≈—œ”¡À» ¿';
				elseif($arLocation['REGION_NAME_ORIG'] == '≈‚ÂÈÒÍ‡ˇ ¿Ó·Î')
					$arLocation['REGION_NAME_ORIG']  = '≈¬–≈…— ¿ﬂ ¿¬“ŒÕŒÃÕ¿ﬂ Œ¡À¿—“‹';
				elseif($arLocation['REGION_NAME_ORIG'] == 'ÕÂÌÂˆÍËÈ ¿Œ')
					$arLocation['REGION_NAME_ORIG']  = 'Õ≈Õ≈÷ »… ¿¬“ŒÕŒÃÕ€… Œ –”√';
				elseif($arLocation['REGION_NAME_ORIG'] == '—Â‚ÂÌ‡ˇ ŒÒÂÚËˇ - ¿Î‡ÌËˇ –ÂÒÔ')
					$arLocation['REGION_NAME_ORIG']  = '—≈¬≈–Õ¿ﬂ Œ—≈“»ﬂ-¿À¿Õ»ﬂ –≈—œ”¡À» ¿';
				elseif($arLocation['REGION_NAME_ORIG'] == '’‡ÌÚ˚-Ã‡ÌÒËÈÒÍËÈ ¿‚ÚÓÌÓÏÌ˚È ÓÍÛ„ - ﬁ„‡ ¿Œ' || $arLocation['REGION_NAME_ORIG'] == '’‡ÌÚ˚-Ã‡ÌÒËÈÒÍËÈ ‡‚ÚÓÌÓÏÌ˚È ÓÍÛ„')
					$arLocation['REGION_NAME_ORIG']  = '’¿Õ“€-Ã¿Õ—»…— »…-ﬁ√–¿ ¿¬“ŒÕŒÃÕ€… Œ –”√';
				elseif($arLocation['REGION_NAME_ORIG'] == '◊ÛÍÓÚÒÍËÈ ¿Œ')
					$arLocation['REGION_NAME_ORIG']  = '◊” Œ“— »… ¿¬“ŒÕŒÃÕ€… Œ –”√';
				elseif($arLocation['REGION_NAME_ORIG'] == 'ﬂÏ‡ÎÓ-ÕÂÌÂˆÍËÈ ¿Œ')
					$arLocation['REGION_NAME_ORIG']  = 'ﬂÃ¿ÀŒ-Õ≈Õ≈÷ »… ¿¬“ŒÕŒÃÕ€… Œ –”√';
				elseif($arLocation['REGION_NAME_ORIG'] == ' ˚Ï')
					$arLocation['REGION_NAME_ORIG']  = ' –€Ã –≈—œ”¡À» ¿';

				$arLocation['REGION_NAME_ORIG'] = preg_replace('/\sŒ¡À$/i'.BX_UTF_PCRE_MODIFIER, ' Œ¡À¿—“‹', ToUpper($arLocation['REGION_NAME_ORIG']));
				$arLocation['REGION_NAME_ORIG'] = preg_replace('/\s–≈—œ$/'.BX_UTF_PCRE_MODIFIER, ' –≈—œ”¡À» ¿', ToUpper($arLocation['REGION_NAME_ORIG']));
				$arLocation['REGION_NAME_ORIG'] = preg_replace('/^(–≈—œ”¡À» ¿)\s*(.*)$/'.BX_UTF_PCRE_MODIFIER, '$2 $1', ToUpper($arLocation['REGION_NAME_ORIG']));
	...
	{
		return
			(ToUpper($arLocation["COUNTRY_NAME_ORIG"]) == "–Œ——»ﬂ"
			|| ToUpper($arLocation["COUNTRY_SHORT_NAME"]) == "–Œ——»ﬂ"
			|| ToUpper($arLocation["COUNTRY_NAME_LANG"]) == "–Œ——»ﬂ"
			...
			|| ToUpper($arLocation["COUNTRY_NAME_ORIG"]) == "–Œ——»…— ¿ﬂ ‘≈ƒ≈–¿÷»ﬂ"
			|| ToUpper($arLocation["COUNTRY_SHORT_NAME"]) == "–Œ——»…— ¿ﬂ ‘≈ƒ≈–¿÷»ﬂ"
			|| ToUpper($arLocation["COUNTRY_NAME_LANG"]) == "–Œ——»…— ¿ﬂ ‘≈ƒ≈–¿÷»ﬂ"
