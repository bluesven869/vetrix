...
class CRestUtil
{
	...
	protected static function getSpecialUrl($method, $query, \CRestServer $server)
	{
		...
		else
		{
			...
			return \CHTTP::URN2URI(
					\Bitrix\Main\Config\Option::get('rest', 'server_path', '/rest')."/".$method.".".$server->getTransport()
	...
	{
		return \CHTTP::URN2URI(\Bitrix\Main\Config\Option::get('rest', 'rest_server_path', '/rest').'/'.urlencode($userId).'/'.urlencode($ap).'/'.($method === '' ? '' : urlencode($method).'/'));
	}
